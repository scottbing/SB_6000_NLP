...
def getNextStory(idx):
    ...
    for widget in originalframe.winfo_children():
        widget.destroy()
    for widget in madlibsframe.winfo_children():
        widget.destroy()

    # load the story into the frame
    left = Text(originalframe, height=10, width=50, wrap=WORD)
    left.configure(font=("Times New Roman", 18, "bold"))
    left.insert(END, text[idx])
    left.pack()
...
getButton = Button(originalframe, text="Get a Story", command=getNextStory(idx))
